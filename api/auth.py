...
def _public_demo_path(request: Request) -> bool:
    return request.url.path in {"/scan", "/scan/url", "/history", "/analyze"}


def _demo_request(request: Request) -> bool:
    return _get_header(request, "x-guni-demo").strip().lower() in {"1", "true", "yes"}
...
def _verify_api_key_from_request(request, api_key: str | None = None, *, allow_open_demo: bool = False) -> str:
    ...
    if (
        allow_open_demo
        and not api_key
        and _public_demo_path(request)
        and (_public_demo_allowed() or _demo_request(request))
    ):
        return "open"
