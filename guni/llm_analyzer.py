...
# Provider responses can take longer than a typical API request when the page
# context is large or the provider is under load.  This applies to connection
# and response reads performed by ``urllib``.
DEFAULT_LLM_TIMEOUT_SECONDS = 60.0


def _llm_timeout_seconds() -> float:
    """Return the configured LLM request/read timeout with safe bounds."""
    raw_timeout = os.environ.get("GUNI_LLM_TIMEOUT_SECONDS", "").strip()
    if not raw_timeout:
    ...
    return timeout if 0 < timeout <= 300 else DEFAULT_LLM_TIMEOUT_SECONDS
...
def _llm_ssl_context() -> ssl.SSLContext | None:
    """Return an optional SSL context for LLM HTTPS requests."""
    ca_bundle = os.environ.get("GUNI_LLM_CA_BUNDLE", "").strip()
    if ca_bundle:
        return ssl.create_default_context(cafile=ca_bundle)
    if not _env_flag("GUNI_LLM_TLS_VERIFY"):
...
def _build_context(parsed_dom: dict, goal: str, heuristic_findings: dict) -> str:
    ...
    # Give Gemini enough surrounding content to reason about page intent while
    # retaining bounded request size for predictable latency and cost.
    visible = parsed_dom.get("visible_text", "")[:12000]
    if visible.strip():
        parts.append(f"VISIBLE TEXT:\n{visible}")

    hidden = parsed_dom.get("hidden_elements", [])
    if hidden:
        hidden_texts = [el.get("text", "")[:800] for el in hidden[:20]]
    ...
    if forms:
        summaries = [
            ...
            for item in forms[:12]
        ]
        parts.append("FORMS:\n" + "\n".join(summaries))

    scripts = parsed_dom.get("scripts", [])
    if scripts:
        preview = "\n".join(script[:1000] for script in scripts[:5])
