...
class GuniScanner:
    ...
    def __init__(
        ...
        self.api_key     = (
            llm_api_key
            or api_key
            or os.environ.get("GUNI_LLM_API_KEY", "")
        ...
        self.llm_provider = (
            llm_provider
            or os.environ.get("GUNI_LLM_PROVIDER", "")
            or ("gemini" if os.environ.get("GEMINI_API_KEY", "") or os.environ.get("GOOGLE_API_KEY", "") else "")
        )
    ...
    def scan(self, html: str, url: str = "") -> dict:
        ...
        should_use_llm = bool(self.api_key and self.llm)
        ...
        if should_use_llm:
            ...
            if not llm_analysis.get("error"):
                # The AI assessment is authoritative when available. Rules stay
                # as a fast fallback and provide grounded evidence to Gemini.
                llm_risk   = llm_analysis.get("overall_risk", 0)
                final_risk = min(100, max(0, llm_risk))
